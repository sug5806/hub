...
from tkinter import * # GUI를 이용할수 있도록 함
from tkinter.filedialog import *  # 파일 경로를 GUI를 사용할 수 있게해줌
from tkinter.simpledialog import * # 다이얼로그를 띄우기 위함.
from wand.image import * # ImageMagic에 있는 기능을 쓰기 위함.
...
def displayImage(img,width,height):#이미지 출력
    ...
    for i in range(0, width):
        for k in range(0, height):
            ...
            # 현실에서 표현하는 x ,y좌표와 이미지상에서 표현하는 좌표는 y,x좌표이므로 x와 y를 바꾸어 준다.
...
def funcOpen():#파일 열기 
    ...
    readFp = askopenfilename(parent= window,filetypes=(("모든 그림 파일","*.jpg;*.jpex;*.bmp;*.png;*.tif;*.gif"),("모든 파일","*.*")))
    while not readFp:
        return
...
def funcSaturation():# 사진 채도
    ...
    label2.pack(side = BOTTOM)
    newPhoto.modulate(100,value,100)
    newX = newPhoto.width
    newY = newPhoto.height
    displayImage(newPhoto,newX,newY)
